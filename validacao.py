from gerador import semestres
def validar_fase1(COMPETIDOR):
    erros = []
    if COMPETIDOR.idade < 17 or COMPETIDOR.idade > 29:
    ...
    if not(COMPETIDOR.semestre in semestres):
        erros.append("Semestre inexistente")
    if COMPETIDOR.universidade in ["UEEST","USPTCC","HARVARD"]:
...
def validar_fase3(COMPETIDOR):
    ...
    if (COMPETIDOR.idade - COMPETIDOR.experiencia) < 17:
